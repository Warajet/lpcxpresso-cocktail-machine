...
void Dispenser::dispense(int ingredientNo, int sec) {
	this->seconds = time(NULL);
	ingredientStartTime[ingredientNo] = seconds;
	ingredientStopTime[ingredientNo] = seconds + sec;
}
...
vector< pair<string, pair<float, int> > > Dispenser :: calculate_each_pump(Recipe* recipe,float quantity)
{
	 ...
	 for(int i = 0; i < recipe_portion_list.size(); i++)
	 {
		 ...
		 int time_taken = ceil(qty / flowrate);
...
void Dispenser :: dispense(Recipe* recipe,float quantity)
{
	 ...
	 for(int i = 0; i < recipe_portion_list.size(); i++)
	 {
		 ...
		 result_components.push_back(tmp);
		 int time_taken = ceil(qty / flowrate);		//Round up to nearest integer
...
void Dispenser::run() {
	...
	for (int i = 0; i < ingredients.size(); i++) {
		if (ingredientStopTime[i] > seconds) {
			ingredients[i]->getPWMOut().write(1);
		} else {
...
int  Dispenser::getDispenseProgress(Ingredient* ingredient){
	...
	if(ingredientStopTime[index] <= seconds){
		return 100;
	}
	else{
		return 100 - (((float)(ingredientStopTime[index] - seconds) / (ingredientStopTime[index] - ingredientStartTime[index]))*100);
	}
